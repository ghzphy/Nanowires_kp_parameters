...
function energies = E_K_reduce_001(params,kpoints)
    ...
    Ham00 = zeros(4*KL,4*KL);
    Ham01 = zeros(4*KL,4*KL);
    ...
    for k = 1: KL
        ...
        for k1 = 1: KL
            ...
            H00_21 = H00_12;
            Ham00_valley001 = [H00_11,H00_12;H00_21,H00_22];
            ...
            H01_12 = 0;
            H01_21 = H01_12;
            Ham01_valley001 = [H01_11,H01_12;H01_21,H01_22];
            %Ham01(2*k-1:2*k,2*k1-1:2*k1) = [H01_11,H01_12;H01_21,H01_22];
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            
            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%Valley 010%%%%%%%%%%%%%%%%%%%%%%%%%%
            H00_11 = (mt_inv*2/(dz^2) + mt_inv*kp^2 + ml_inv*kq^2 + U)*delta(p,p1)*delta(q,q1);
            H00_22 = (mt_inv*2/(dz^2) + mt_inv*kp^2 + ml_inv*kq^2 + U)*delta(p,p1)*delta(q,q1);
            if q ~= q1
                H00_11 = H00_11 + 2i*ml_inv*k0*(4*kq1/pi*q/(q^2-q1^2)*delta_odd(q,q1))*delta(p,p1);
                H00_22 = H00_22 - 2i*ml_inv*k0*(4*kq1/pi*q/(q^2-q1^2)*delta_odd(q,q1))*delta(p,p1);
            end
            H00_12 = 0;H00_21 = H00_12;
            Ham00_valley010 = [H00_11,H00_12;H00_21,H00_22];
            
            % H11 = 
            H01_11 = (-mt_inv/(dz^2))*delta(p,p1)*delta(q,q1);
            % H22 = 
            H01_22 = (-mt_inv/(dz^2))*delta(p,p1)*delta(q,q1);
            % H12 = -2*kx*ky/M
            if p == p1
                H01_12 = 0;
            else
                H01_12 = M_inv/dz*(4*kp1/pi*p/(p^2-p1^2)*delta_odd(p,p1)*delta(q,q1));
            end
            H01_21 = H01_12;
            Ham01_valley010 = [H01_11,H01_12;H01_21,H01_22];
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            
            Ham00(4*k-3:4*k,4*k1-3:4*k1) = blkdiag(Ham00_valley001, Ham00_valley010);
            Ham01(4*k-3:4*k,4*k1-3:4*k1) = blkdiag(Ham01_valley001, Ham01_valley010);
        end
    end
